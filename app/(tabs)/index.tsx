...
import { WhatToTakeSection } from '@/components/WhatToTakeSection';
...
export default function HomeScreen() {
    ...
    const todayWorkout = useMemo(() => {
        if (fetchedDays.length === 0) {
            if (isLoading) {
                return { title: 'Loading...', exercises: [], dayNumber: 0, anatomyFocus: [], warmup: [], cooldown: "", whatToTake: [] };
            }
            ...
            try {
                ...
                return {
                    ...
                    whatToTake: fallbackWorkout.whatToTake || []
                };
            } catch (e) {
                return { title: 'Rest Day', exercises: [], dayNumber: effectiveToday, anatomyFocus: ["abs"], warmup: [], cooldown: "", whatToTake: [] };
            }
        }
        ...
        return {
            ...
            warmup: workoutForDay.warmup || [],
            cooldown: workoutForDay.cooldown || "",
            whatToTake: workoutForDay.whatToTake || []
        };
    ...
    const [activeRest, setActiveRest] = useState<{ exerciseName: string; seconds: number } | null>(null);

    const parseRestSeconds = (restStr: string): number => {
        const clean = restStr.toLowerCase().trim();
        if (clean.includes('min')) {
            const num = parseFloat(clean.replace('min', ''));
            return Math.round(num * 60);
        }
        if (clean.includes('s')) {
            return parseFloat(clean.replace('s', '')) || 0;
        }
        return parseFloat(clean) || 0;
    };

    const handleTriggerRest = useCallback((exerciseName: string, restDuration: string) => {
        const secs = parseRestSeconds(restDuration);
        if (secs > 0) {
            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
            setActiveRest({ exerciseName, seconds: secs });
        }
    }, []);
        ...
        return (
            <Animated.View entering={FadeInDown.delay(index * 75).duration(400).springify().damping(25).stiffness(250)}>
                <ExerciseCard
                    ...
                    onTriggerRest={handleTriggerRest}
                />
            </Animated.View>
        );
    }, [completedExercises, loggedWeights, handleToggleComplete, handleUpdateWeight, handleTriggerRest]);
                                ...
                                ListHeaderComponent={
                                    <View>
                                        {todayWorkout.warmup && todayWorkout.warmup.length > 0 && (
                                            <WarmupSection warmup={todayWorkout.warmup} />
                                        )}
                                        {todayWorkout.whatToTake && todayWorkout.whatToTake.length > 0 && (
                                            <WhatToTakeSection whatToTake={todayWorkout.whatToTake} />
                                        )}
                                    </View>
                                }
                                ListFooterComponent={
                                    todayWorkout.exercises.length > 0 ? (
                                        <View>
                                            {todayWorkout.cooldown ? (
                                                <View style={{ backgroundColor: appTheme.colors.backgroundCard, borderWidth: 1, borderColor: appTheme.colors.border, padding: 16, marginBottom: 20 }}>
                                                    <Text style={{ color: appTheme.colors.accentSecondary || '#FF3300', fontFamily: appTheme.typography.fontFamily.monoBold, fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>
                                                        [ AFTER_WORKOUT_COOLDOWN ]
                                                    </Text>
                                                    <Text style={{ color: appTheme.colors.textPrimary, fontFamily: appTheme.typography.fontFamily.heading, fontSize: 18, textTransform: 'uppercase', marginBottom: 8 }}>
                                                        COOL-DOWN & STRETCHING
                                                    </Text>
                                                    <Text style={{ color: appTheme.colors.textSecondary, fontFamily: appTheme.typography.fontFamily.mono, fontSize: 12, lineHeight: 18 }}>
                                                        {todayWorkout.cooldown}
                                                    </Text>
                                                </View>
                                            ) : null}
