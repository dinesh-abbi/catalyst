        ...
        <View style={{ backgroundColor: appTheme.colors.backgroundCard, borderColor: 'rgba(51, 65, 85, 0.4)', borderWidth: 1 }} className="rounded-[24px] p-6 mb-4">
            ...
            <View className="flex-row items-center justify-between bg-[#0b0f19] rounded-[18px] p-4 mb-4 border border-slate-800/30">
                <View className="flex-row items-center flex-1">
                    ...
                    {exercise.rest && (
                        <View>
                            <Text style={{ color: appTheme.colors.textTertiary, fontFamily: appTheme.typography.fontFamily.monoBold }} className="text-[10px] tracking-widest uppercase mb-1">REST</Text>
                            <Text style={{ color: appTheme.colors.accent, fontFamily: appTheme.typography.fontFamily.monoBold }} className="text-lg">{exercise.rest}</Text>
                        </View>
                    )}
                </View>
...
export default function WeeklyOverviewScreen() {
    ...
    const handleSaveReorderedSplit = async (newDays: WorkoutDayRaw[]) => {
        try {
            setCustomWorkoutDays(newDays);
            setIsReorderModalVisible(false);
            showAlert('SUCCESS', 'Workout split reordered and synchronized!', 'SUCCESS');
            
            // Upload to Firestore in background
            saveCustomExercises(newDays).catch((err: any) => {
                console.warn("Failed to upload custom workout split to Firestore:", err);
            });
        } catch (e: any) {
            showAlert('ERROR', 'Failed to save reordered workout split.', 'ERROR');
        }
    };
    ...
    return (
        <SafeAreaView className="flex-1" style={{ backgroundColor: appTheme.colors.backgroundMain }}>
                ...
                <Animated.View entering={FadeInDown.delay(100).duration(600)} style={{ marginBottom: 20 }}>
                    <View style={{
                        ...
                        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 10 }}>
                            ...
                            >
                                <Feather name="sliders" size={12} color={appTheme.colors.accentTertiary || '#00E5FF'} />
                                <Text style={{ color: appTheme.colors.accentTertiary || '#00E5FF', fontFamily: appTheme.typography.fontFamily.monoBold, fontSize: 10, letterSpacing: 1 }}>{"[ REORDER_SHUFFLE ]"}</Text>
                            </TouchableOpacity>
                        </View>
                        ...
                        <FlatList
                            ...
                            ListHeaderComponent={
                                <View>
                                    ...
                                    {selectedDay?.warmup && selectedDay.warmup.length > 0 && (
                                        <View style={{ backgroundColor: 'rgba(255,255,255,0.02)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.05)', padding: 16, marginBottom: 20 }}>
                                            <Text style={{ color: appTheme.colors.accent, fontFamily: appTheme.typography.fontFamily.monoBold, fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 8 }}>
                                                [ WARMUP PROTOCOL ]
                                            </Text>
                                            {selectedDay.warmup.map((w, idx) => (
                                                <View key={idx} style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                                                    <View style={{ flex: 1, marginRight: 10 }}>
                                                        <Text style={{ color: appTheme.colors.textPrimary, fontFamily: appTheme.typography.fontFamily.heading, fontSize: 13, textTransform: 'uppercase' }}>{w.name}</Text>
                                                        <Text style={{ color: appTheme.colors.textSecondary, fontFamily: appTheme.typography.fontFamily.mono, fontSize: 10 }}>{w.focus}</Text>
                                                    </View>
                                                    <Text style={{ color: appTheme.colors.accent, fontFamily: appTheme.typography.fontFamily.monoBold, fontSize: 11 }}>{w.duration}</Text>
                                                </View>
                                            ))}
                                        </View>
                                    )}
                                    ...
                                    {/* What to Take */}
                                    {selectedDay?.whatToTake && selectedDay.whatToTake.length > 0 && (
                                        <View style={{ backgroundColor: 'rgba(0, 229, 255, 0.05)', borderWidth: 1, borderColor: 'rgba(0, 229, 255, 0.1)', padding: 16, marginBottom: 24 }}>
                                            <Text style={{ color: appTheme.colors.accentTertiary || '#00E5FF', fontFamily: appTheme.typography.fontFamily.monoBold, fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 8 }}>
                                                [ DIET & SUPPLEMENT PREP ]
                                            </Text>
                                            {selectedDay.whatToTake.map((t, idx) => (
                                                <View key={idx} style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 6 }}>
                                                    <Text style={{ color: appTheme.colors.accentTertiary || '#00E5FF', marginRight: 8 }}>•</Text>
                                                    <Text style={{ color: appTheme.colors.textPrimary, fontFamily: appTheme.typography.fontFamily.heading, fontSize: 13, flex: 1, textTransform: 'uppercase' }}>{t}</Text>
                                                </View>
                                            ))}
                                        </View>
                                    )}
                            ...
                            ListFooterComponent={
                                selectedDay?.cooldown ? (
                                    <View style={{ backgroundColor: 'rgba(255, 51, 0, 0.05)', borderWidth: 1, borderColor: 'rgba(255, 51, 0, 0.1)', padding: 16, marginTop: 10, marginBottom: 40 }}>
                                        <Text style={{ color: appTheme.colors.accentSecondary || '#FF3300', fontFamily: appTheme.typography.fontFamily.monoBold, fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 6 }}>
                                            [ AFTER_WORKOUT_COOLDOWN ]
                                        </Text>
                                        <Text style={{ color: appTheme.colors.textPrimary, fontFamily: appTheme.typography.fontFamily.heading, fontSize: 16, textTransform: 'uppercase', marginBottom: 6 }}>
            ...
            <ReorderModal
                ...
                currentDays={customWorkoutDays && customWorkoutDays.length > 0 ? customWorkoutDays : workoutData}
                onSave={handleSaveReorderedSplit}
            />
        </SafeAreaView>
    );
}
