...
export const useWorkoutStore = create<WorkoutState>()(
    persist(
        (set, get) => ({
            ...
            workoutTemplateVersion: "pull_width_v2",
        ...
        }),
        {
            ...
            partialize: (state) => ({
                ...
                workoutTemplateVersion: state.workoutTemplateVersion,
            }),
            onRehydrateStorage: () => (state) => {
                // As soon as the store rehydrates from AsyncStorage, check if it's a new day
                if (state) {
                    state.checkAndResetAtMidnight();
                    // Migrate custom splits if workout template has updated
                    const currentVersion = "pull_width_v2";
                    if (state.workoutTemplateVersion !== currentVersion) {
                        console.log("[store] Default workout template updated. Resetting custom splits...");
                        state.setCustomWorkoutDays(null);
                        state.setScheduleOffset(0);
                        // Delete custom exercises from Firestore in background
                        try {
                            const { clearCustomExercises } = require('@/utils/WorkoutService');
                            clearCustomExercises().catch((err: any) => {
                                console.warn("[store] Failed to clear custom exercises in Firestore:", err);
                            });
                        } catch (e) {
                            console.warn("[store] Failed to clear custom exercises:", e);
                        }
                        // Update stored template version
                        state.workoutTemplateVersion = currentVersion;
                    }
                }
            },
        }
